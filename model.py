...
X = new_train.drop("att_flag", axis=1)
y = new_train["att_flag"]

X_test = new_test.drop("att_flag", axis=1)
y_test = new_test["att_flag"]
...
print("Training f1 score {}".format(
    f1_score(y, y_train_pred, pos_label=1)))
print("Evaluation f1 score {}".format(
...
print("Evaluation f1 score {}".format(
    f1_score(y_test, y_knn_predict, pos_label=1)))
...
def svm_clf():
    ...
    # predict
    pred_y = clf_svc.predict(x_t)

    # anlayse results
    c_matrix = confusion_matrix(y_t, pred_y)
    error = zero_one_loss(y_t, pred_y)
    score = accuracy_score(y_t, pred_y)
    ...
    disp = plot_confusion_matrix(clf_svc, x_t, y_t, cmap=plt.cm.Greens, values_format='.0f',
...
def logistic_reg_clf():
    ...
    # Plot non-normalized confusion matrix
    disp = plot_confusion_matrix(clf_lr, x_t, y_t, cmap=plt.cm.Greens, values_format='.0f',
                                 xticks_rotation='horizontal')
...
X_train = new_train.drop("att_cat", axis=1)
y = new_train["att_cat"]
...
#X_t, X_val, Y_t, Y_val = train_test_split(X_train, Y_train, test_size = 0.1, random_state=random_seed)
...
preds = model.predict(X_t)  # see how the model did!
...
matrix = confusion_matrix(dummy_y_t.argmax(axis=1), preds.argmax(axis=1))
matrix

pr = preds.argmax(axis=1)


y_unique = y_t.unique()
mcm = multilabel_confusion_matrix(y_t, pr, labels=y_unique)
mcm
